...
entradaDeDados.question("Digite o nome do cliente: ", function (cliente) {
    ...
    entradaDeDados.question("Qual o produto que foi comprado: ", function (produto) {
        ...
        entradaDeDados.question("Qual a quantidade do produto?", function(qtde){
                    ...
                    entradaDeDados.question("Será pago em meses ou anos: ", function (situação) {
                        ...
                        if (situação == "meses" || situação == "Meses") {
                            ...
                            entradaDeDados.question("Quantos meses será necessario: ", function (meses) {
                                ...
                                if (nomeCliente == "" || nomeProduto == "" || valorCompra == "" || jurosComposto == "" || parcelasMeses == "" || quantidadeDeProduto == "") {
                                    console.log("ERRO: CAMPOS OBRIGATÓRIOS")
                                } else if (valorCompra < 0 || jurosComposto < 0 || parcelasMeses < 0 || quantidadeDeProduto < 0) {
                                    console.log("ERRO: NÚMERO INFERIOR A 0")
                                } else if (isNaN(valorCompra) || isNaN(jurosComposto) || isNaN(parcelasMeses) || isNaN(quantidadeDeProduto)) {
                                    ...
                                    let ValorDaQuantidade = Number(quantidadeDeProduto) * Number(valorCompra)
                                    let valor = Number(ValorDaQuantidade) * ((Number(1) + (Number(jurosComposto) / 100)) ** Number(parcelasMeses))
                                    let acrescimo = Number(valor) - Number(ValorDaQuantidade)
    
                                    console.log(`
                                        \n************************* Viva Moda ***************************
                                        \nMuito obrigado por realizar a sua compra conosco Sr(a)${nomeCliente}.
                                        \nA compra do produto ${nomeProduto}, tem um valor de: R$${ValorDaQuantidade}.
                                        \nA sua compra será parcelada em ${parcelasMeses} vezes e o Sr(a) pagará: ${valor.toFixed(2)}
                                        \nO acréscimo realizado ao valor de: R$${ValorDaQuantidade} será de R$${acrescimo.toFixed(2)}..
                            ...
                            entradaDeDados.question("Quantos anos será necessario: ", function (anos) {
                                ...
                                if (nomeCliente == "" || nomeProduto == "" || valorCompra == "" || jurosComposto == "" || parcelasEmAnos == "") {
                                    ...
                                    let ValorDaQuantidade = Number(quantidadeDeProduto) * Number(valorCompra)
                                    let valor = Number(ValorDaQuantidade) * ((Number(1) + (Number(jurosComposto) / 100)) ** (Number(parcelasEmAnos) * 12))
                                    let acrescimo = Number(valor) - Number(ValorDaQuantidade)
                                    console.log(`
                                        \n************************* Viva Moda ***************************
                                        \nMuito obrigado por realizar a sua compra conosco Sr(a)${nomeCliente}.
            ...
            }) // fecha valor da compras
        }) // fecha quantidade do produto
    }) // fecha produtos
}) // fecha cliente